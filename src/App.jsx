...
import {
  ...
  AlertTriangle, ShieldCheck, FileSearch, ChevronRight,
...
function ScannerPage({ onSendToAI, onSendToChat }) {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {tab === 'analyze' && (() => {
            ...
            return (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                {/* F2 — UPX Unpack */}
                {result.packers?.includes('UPX') || result.sections?.some(s => s.name.toLowerCase().startsWith('.upx')) ? (
                  <Card>
                    <CardHeader>UPX Packer Detected (F2) — Auto-Unpack</CardHeader>
                    <div style={{ padding: '12px 16px' }}>
                      <div style={{ fontSize: 12, color: '#f59e0b', marginBottom: 10 }}>
                        UPX packed executable detected. Click below to attempt automatic decompression using system <code style={{ fontSize: 10, background: 'rgba(255,255,255,0.06)', padding: '2px 5px', borderRadius: 4 }}>upx</code> tool.
                      </div>
                      <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                        <button disabled={upxRunning || !scanFilePath} onClick={async () => {
                          ...
                          try { const r = await invoke('unpack_upx', { filePath: scanFilePath }); setUpxResult(r); }
                          ...
                          {upxRunning ? <><Spinner /> Unpacking⬦</> : '⚡ Run upx -d'}
                        </button>
                      ...
                      {upxResult && (
                        <div style={{ marginTop: 10, padding: '8px 12px', borderRadius: 7, background: upxResult.ok ? 'rgba(34,197,94,0.06)' : 'rgba(239,68,68,0.06)', border: `1px solid ${upxResult.ok ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.2)'}` }}>
                          <div style={{ fontSize: 11, color: upxResult.ok ? '#4ade80' : '#f87171', marginBottom: 4 }}>{upxResult.ok ? '✅ Unpacked successfully — file overwritten in-place' : '❌ Unpack failed'}</div>
                ...
                {/* F3 — Memory Dump Analysis */}
                {(() => {
                  const isDump = file?.name?.match(/\.(dmp|dump|mem|bin|raw)$/i) || (result.overallEntropy > 5.0 && !result.isPe);
                  if (!isDump) return null;
                  return (
                    <Card>
                      <CardHeader>Memory Dump / Raw Binary Analysis (F3)</CardHeader>
                      <div style={{ padding: '12px 16px' }}>
                        <div style={{ fontSize: 12, color: '#60a5fa', marginBottom: 10 }}>
                          Detected memory dump or raw binary. Click to run deep analysis: embedded PE search, entropy mapping, and string extraction.
                        </div>
                        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', marginBottom: dumpResult ? 12 : 0 }}>
                          <button disabled={dumpRunning || !scanFilePath} onClick={async () => {
                            ...
                            try { const r = await invoke('analyze_dump', { filePath: scanFilePath }); setDumpResult(r); }
                            ...
                            {dumpRunning ? <><Spinner /> Analyzing⬦</> : '🔬 Analyze Dump'}
                          </button>
                        ...
                        {dumpResult && !dumpResult.error && (
                          <div>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 8, marginBottom: 10 }}>
                              ...
                              <div style={{ padding: '8px 12px', borderRadius: 7, background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.14)' }}>
                                <div style={{ fontSize: 10, color: '#4b5563', marginBottom: 2 }}>Embedded PE headers</div>
                                <div style={{ fontSize: 13, fontWeight: 700, color: dumpResult.pe_offsets?.length > 0 ? '#f59e0b' : '#94a3b8', fontFamily: 'monospace' }}>{dumpResult.pe_offsets?.length || 0}</div>
                              </div>
                              <div style={{ padding: '8px 12px', borderRadius: 7, background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.14)' }}>
                                <div style={{ fontSize: 10, color: '#4b5563', marginBottom: 2 }}>Likely dump?</div>
                                <div style={{ fontSize: 13, fontWeight: 700, color: dumpResult.is_likely_dump ? '#f59e0b' : '#4ade80' }}>{dumpResult.is_likely_dump ? 'Yes' : 'No'}</div>
                              </div>
                            </div>
                            {dumpResult.pe_offsets?.length > 0 && (
          ...
          {tab === 'compare' && compareResult && !compareResult.error && (() => {
            ...
            return (
              ...
              {/* B6 — Byte-level Hex Diff */}
              {scanRawBytes && compareRawBytes && (() => {
                const LEN = Math.min(scanRawBytes.length, compareRawBytes.length, 512);
                let diffCount = 0;
                for (let i = 0; i < LEN; i++) if (scanRawBytes[i] !== compareRawBytes[i]) diffCount++;
                const COLS = 16;
                const rows = Math.ceil(LEN / COLS);
                return (
                  <Card style={{ marginTop: 12 }}>
                    <CardHeader>Byte-Level Hex Diff (B6) — First {LEN} bytes · {diffCount} differences ({((diffCount/LEN)*100).toFixed(1)}% changed)</CardHeader>
          ...
          <div key={p.id}>
            ...
            {hexData?.patchId === p.id && (
              <div style={{ margin: '4px 0 8px 0', borderRadius: 8, overflow: 'hidden', border: '1px solid rgba(245,158,11,0.2)', background: 'rgba(0,0,0,0.3)' }}>
                <div style={{ padding: '4px 12px', background: 'rgba(245,158,11,0.07)', fontSize: 10, color: '#f59e0b', fontWeight: 600, fontFamily: 'monospace' }}>
                  offset {hexData.offset} · 128 bytes
                </div>
...
function ChatPage({ preloaded, scanHistory }) {
  ...
  useEffect(() => {
    if (!preloaded) return;
    // PE Analyst -- put context into chat input
    if (preloaded.type === 'pe_analyst') {
      const { type, ...rest } = preloaded;
      setInput(`[PE/Binary Analizi]\n\n${JSON.stringify(rest, null, 2)}\n\nBu analiz hakkında ne söyleyebilirsin?`);
      return;
    }
    const labels = { scanner: 'Binary Scanner', patcher: 'Hex Patcher', ai_analyst: 'AI Analyst raporu' };
    const label  = labels[preloaded.type] || 'Veri';
    let text = '';
    if (preloaded.type === 'scanner') {
      const d = preloaded.data;
      text = `[PE/Binary Analizi - Tarama Sonucu]\n\nDosya: ${preloaded.fileName || '?'}\nMimari: ${d.arch}  EP: 0x${d.ep?.toString(16).toUpperCase()}  Risk: ${d.riskScore}/100\nDenuvo: ${d.denuvo ? 'EVET' : 'Hay\u0131r'}  VMProtect: ${d.vmp ? 'EVET' : 'Hay\u0131r'}  Themida: ${d.themida ? 'EVET' : 'Hay\u0131r'}\nGenel entropi: ${d.overallEntropy?.toFixed(3)} H  \u015e\u00fcpheli section: ${d.suspiciousCount}\n\nSectionlar:\n${d.sections?.map(s => `  ${s.name.padEnd(12)} entropi=${s.entropy.toFixed(3)} boyut=${(s.rsize/1024).toFixed(0)}KB${s.suspicious ? ' \u26a0' : ''}`).join('\n')}\n\nBu sonu\u00e7lara g\u00f6re neler s\u00f6yleyebilirsin?`;
    } else if (preloaded.type === 'patcher') {
      const patches = preloaded.data;
      text = `[Hex Patch Listesi]\n\nHedef: ${preloaded.fileName || 'Belirtilmemi\u015f'}\n\n${patches.map(p => `- ${p.name}\n  Offset: ${p.offset}\n  Orijinal: ${p.original || '\u2014'}  \u2192  Yama: ${p.patched || '\u2014'}\n  Durum: ${p.applied ? 'Uygulnad\u0131' : p.enabled ? 'Aktif' : 'Devre d\u0131\u015f\u0131'}`).join('\n\n')}\n\nBu patch'lerin amac\u0131 ve olas\u0131 etkileri nelerdir?`;
    } else if (preloaded.type === 'ai_analyst') {
      text = `AI Analyst \u015fu raporu \u00fcretti:\n\n${preloaded.data.output}\n\nBu rapor hakk\u0131nda sorular\u0131m var, konu\u015falm.`;
    }
    setInput(text);
  }, [preloaded]);
  ...
  const sendMessage = async () => {
    ...
    try {
      await invoke('lms_chat_stream', {
        messages: [
          { role: 'system', content: curMode?.system || AI_MODES[1].system },
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {preloaded && messages.length === 0 && !streaming && (
          <div style={{ borderRadius: 10, padding: '10px 14px', background: 'rgba(139,92,246,0.06)', border: '1px solid rgba(139,92,246,0.2)', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ fontSize: 11, color: '#6b7280' }}>
              <strong style={{ color: '#a78bfa' }}>{{ scanner: 'Binary Scanner', patcher: 'Hex Patcher', ai_analyst: 'AI Analyst' }[preloaded.type] || ''}</strong> verisi yüklendi — textarea dolduruldu, gönder butonuna bas.
            </div>
          </div>
        )}
        {messages.length === 0 && !streaming && !preloaded && (
...
const BUILTIN_PLUGINS = [
  { id: 'pe_scanner',   name: 'PE Scanner',        desc: 'PE başlık, entropy, section, string, import analizi.',      icon: <ShieldAlert size={15} />, version: '2.0.0', status: 'active', builtin: true  },
  { id: 'hex_patcher',  name: 'Hex Patcher',        desc: 'İkili offset yamalama, NOP injection, backup sistemi.',     icon: <Binary size={15} />,       version: '2.0.0', status: 'active', builtin: true  },
  { id: 'yara_engine',  name: 'YARA Engine',        desc: 'JS tabanlı davranışsal kural motoru, 15 yerleşik kural.',   icon: <FileSearch size={15} />,   version: '1.0.0', status: 'active', builtin: true  },
  ...
  { id: 'binary_diff',  name: 'Binary Diff',        desc: 'İki PE dosyasını yan yana karşılaştırma.',                  icon: <Layers size={15} />,       version: '1.0.0', status: 'active', builtin: true  },
  { id: 'hex_viewer',   name: 'Hex Region Viewer',  desc: 'Dosyadan bölge bazlı ham hex okuma (Rust destekli).',       icon: <Terminal size={15} />,     version: '1.0.0', status: 'active', builtin: true  },
  { id: 'sig_scanner',  name: 'Packer Signatures',  desc: 'UPX/MPRESS/ASPack/Petite section+EP imza tanıma.',          icon: <Cpu size={15} />,          version: '1.0.0', status: 'active', builtin: true  },
  { id: 'community_1',  name: 'Disassembler View',  desc: 'Yakında — x86/x64 basit opcode gösterimi.',                 icon: <Code size={15} />,         version: '—',     status: 'soon',  builtin: false },
  { id: 'community_2',  name: 'Syscall Tracer',     desc: 'Yakında — Windows NT syscall ID eşleşme tablosu.',          icon: <Zap size={15} />,          version: '—',     status: 'soon',  builtin: false },
  { id: 'community_3',  name: 'Import Grapher',     desc: 'Yakında — import bağımlılık grafiği görselleştirmesi.',     icon: <Network size={15} />,      version: '—',     status: 'soon',  builtin: false },
];

function PluginPage() {
  const [loadMsg, setLoadMsg] = useState('');
  const active  = BUILTIN_PLUGINS.filter(p => p.status === 'active');
  const soon    = BUILTIN_PLUGINS.filter(p => p.status === 'soon');
  const handleLoad = () => {
    setLoadMsg("Plugin yükleme sistemi v3.0'da aktif olacak.");
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 5 }}>
            ...
            <h1 style={{ fontSize: 20, fontWeight: 700, color: '#e2e8f0', margin: 0 }}>Plugin System</h1>
          </div>
          <p style={{ fontSize: 11, color: '#374151', margin: 0 }}>Yerleşik modüller · Topluluk eklentileri · API genişletme noktaları</p>
        </div>
        <button onClick={handleLoad}
          style={{ fontSize: 11, padding: '7px 14px', borderRadius: 8, border: '1px solid rgba(99,102,241,0.25)', background: 'rgba(99,102,241,0.07)', color: '#818cf8', cursor: 'pointer', fontWeight: 500, display: 'flex', alignItems: 'center', gap: 6 }}>
          <Download size={13} /> Plugin Y?kle
        </button>
      </div>
      {loadMsg && <div style={{ marginBottom: 12, padding: '8px 14px', borderRadius: 8, background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.2)', fontSize: 11, color: '#818cf8' }}>{loadMsg}</div>}

      {/* Active / Built-in */}
      <Card style={{ marginBottom: 16 }}>
        <CardHeader>Aktif Mod?ller — {active.length}</CardHeader>
        <div style={{ padding: '4px 0 8px' }}>
          {active.map(p => (
            <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '9px 16px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: '#e2e8f0' }}>{p.name} <span style={{ fontSize: 9, color: '#374151', fontWeight: 400, fontFamily: 'monospace' }}>v{p.version}</span></div>
              ...
              {p.builtin && <span style={{ fontSize: 9, padding: '2px 7px', borderRadius: 4, background: 'rgba(99,102,241,0.1)', color: '#818cf8', fontWeight: 600 }}>built-in</span>}
            </div>
          ))}
        </div>
      </Card>
      ...
      {/* Coming soon */}
      <Card>
        <CardHeader>Yakında — {soon.length} modül geli?tiriliyor</CardHeader>
        <div style={{ padding: '4px 0 8px' }}>
          {soon.map(p => (
            <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '9px 16px', borderBottom: '1px solid rgba(255,255,255,0.04)', opacity: 0.45 }}>
              <div style={{ width: 30, height: 30, borderRadius: 7, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#374151', flexShrink: 0 }}>{p.icon}</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: '#4b5563' }}>{p.name}</div>
                <div style={{ fontSize: 10, color: '#2d3748', marginTop: 1 }}>{p.desc}</div>
              </div>
              <span style={{ fontSize: 9, padding: '2px 8px', borderRadius: 4, background: 'rgba(251,191,36,0.08)', color: '#fbbf24', fontWeight: 700 }}>SOON</span>
            </div>
          ))}
        </div>
      </Card>
      ...
      {/* API hint card */}
      <div style={{ marginTop: 16, borderRadius: 12, padding: '14px 18px', background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.05)' }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: '#374151', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Plugin API — ?nizleme</div>
        <pre style={{ fontSize: 10, fontFamily: 'monospace', color: '#4b5563', margin: 0, lineHeight: 1.6 }}>
{`// Her plugin bu arayüzü uygular:
export default {
  id: 'my_plugin',
  name: 'My Plugin',
  version: '1.0.0',
  // Hook'lar:
  onScan(result)  { /* PE tarama sonucunu al */ },
  onPatch(file)   { /* Patcher'dan tetiklenir   */ },
  renderPanel()   { /* JSX döndür ? sidebar    */ },
};`}
        </pre>
      </div>
    </div>
...
const VIEWS = { SCANNER: 'scanner', PATCHER: 'patcher', SYSTEM: 'system', CHAT: 'chat', PLUGINS: 'plugins' };
...
export default function App() {
  ...
  useEffect(() => {
    const VLIST = [VIEWS.SCANNER, VIEWS.PATCHER, VIEWS.CHAT, VIEWS.SYSTEM, VIEWS.PLUGINS];
    const handler = (e) => {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', width: '100vw', overflow: 'hidden', background: T.bg, color: '#e6edf3', fontFamily: "'Inter','SF Pro Display',system-ui,sans-serif", userSelect: 'none', transform: zoom !== 1 ? `scale(${zoom})` : undefined, transformOrigin: 'top left', ...(zoom !== 1 ? { width: `${100 / zoom}vw`, height: `${100 / zoom}vh` } : {}) }}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <aside style={{ width: sidebarWidth, flexShrink: 0, background: T.sidebar, borderRight: `1px solid ${T.border}`, display: 'flex', flexDirection: 'column', padding: '14px 8px', position: 'relative', userSelect: sidebarDragging ? 'none' : undefined }}>
          ...
          <NavItem active={view === VIEWS.PLUGINS} onClick={() => setView(VIEWS.PLUGINS)} icon={<Layers size={15} />}     label="Plugins"         sub="Modüller · Genişletme" badge="WIP" />
          <div style={{ flex: 1 }} />
        ...
        <main style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', background: T.bg }}>
          {view === VIEWS.SCANNER && <ScannerPage onSendToAI={sendToAI} onSendToChat={sendToChat} />}
          ...
          <div style={{ flex: 1, display: view === VIEWS.CHAT ? 'flex' : 'none', flexDirection: 'column', overflow: 'hidden' }}>
            <ChatPage preloaded={chatContext} scanHistory={scanHistory} />
          </div>
      ...
      {cmdOpen && (() => {
        const CMD_LIST = [
          ...
          ...getHistory().slice(0, 5).map(h => ({
            ...
            action: () => { /* scanner redirect is handled by preloaded state - just show */ setCmdOpen(false); },
          })),
        ];
